...
// // async function getPosts(): Promise<SanityTypes.Post[]> {
// //   const query = `
// //   *[_type == 'post'] | order(_createdAt desc) {
// //       _id,
// //       title,
...
// //   description: "Explore our latest blog posts and stay updated!",
// // };
...
// //                     />
// //                   ) : (
// //                     <div className="h-full w-full flex items-center justify-center bg-gray-200 dark:bg-gray-700 text-gray-500">
// //                       No Image
// //                     </div>
// //                   )}
// //                 </div>
...
// type Props = {
//   params: { slug: string }; // Add correct typing
...
// export default async function Page({}: Props) { // Removed params
//   const posts: SanityTypes.Post[] = await getPosts();
...
// Define Props for Page
type PageProps = {
  params: { slug: string }; // Correctly typed
  searchParams?: { [key: string]: string | string[] | undefined };
};
  ...
  return (
    <div className="flex flex-col items-center w-full bg-gray-50 dark:bg-gray-900">
      <div className="min-h-[calc(100vh-5rem)] w-full flex flex-1 max-w-[1500px] md:px-14 pt-24 px-4 flex-col space-y-4 pb-36">
        ...
        <div className="grid md:grid-cols-3 gap-8 grid-cols-1">
          {posts.map((post: SanityTypes.Post) => (
            <a
              key={post._id}
              href={`/post/${post.slug?.current || ""}`}
              className="space-y-4 cursor-pointer"
